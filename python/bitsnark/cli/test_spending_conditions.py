    ...
    def run(
        ...
        if context.args.names:
            ...
            tx_template_query = tx_template_query.filter(
                TransactionTemplate.name.in_(names)
            )
        ...
        logger.info(
            "Getting spending conditions from %s tx templates", len(tx_templates)
        )
        ...
        for tx_template in tx_templates:
            for output_index, output in enumerate(tx_template.outputs):
                for spending_condition in output["spendingConditions"]:
                    ...
                    if (
                        not context.args.enable_timelocks
                        and "timeoutBlocks" in spending_condition
                    ):
                        logger.info(
        ...
        results = []
        for test_index, (tx_template, output_index, spending_condition) in enumerate(
            to_test, start=1
        ):
            logger.info(
