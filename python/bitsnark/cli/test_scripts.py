...
from ..core.script_testing import (
    TestCase,
    Result,
...
class TestScriptsCommand(Command):
    ...
    def init_parser(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--filter", help="template_name/output_index/spending_condition_index"
        )
        parser.add_argument(
            "--prover-privkey",
        ...
        parser.add_argument(
            "--debug",
            help="Drop into Python debugger before testing mempoolaccept",
            action="store_true",
        )
        parser.add_argument(
            "--print-script", help="Print each script", action="store_true"
        )
        parser.add_argument(
            "--print-witness",
        ...
        parser.add_argument(
            "--enable-timelocks",
            help="Enable testing of timelock transactions",
            action="store_true",
        )
        parser.add_argument(
            "--eval", help="Evaluate script before submitting", action="store_true"
        )
    ...
    def run(
        ...
        filter_spending_condition_index = (
            int(filter_parts[2]) if len(filter_parts) > 2 else None
        )
        if filter_parts:
            logger.info(
                "Filtering to tx_template: %s, output_index: %s, spending_condition_index: %s",
                filter_name,
                filter_output_index,
                filter_spending_condition_index,
            )
        ...
        if filter_name:
            tx_template_query = tx_template_query.filter(
                TransactionTemplate.name == filter_name
            )
