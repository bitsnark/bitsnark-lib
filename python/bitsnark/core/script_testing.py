...
class TestCase:
    ...
    def script_repr(self, *, limit: int = None, newlines: bool = False):
        ...
        ret = (
            ret.removeprefix("CBitcoinScript([")
            .removesuffix("], name='script')")
            .removesuffix("])")
        )
    ...
    for tx_template in tx_templates:
        ...
        for output_index, output in enumerate(tx_template.outputs):
            ...
            for spending_condition in output["spendingConditions"]:
                if (
                    filter_spending_condition_index is not None
                    and spending_condition["index"] != filter_spending_condition_index
                ):
                    continue
                ...
                test_case = TestCase(
                    script=CScript(
                        parse_hex_bytes(spending_condition["script"]), name="script"
                    ),
...
def execute_script_test_case(
    *,
    ...
    internal_pubkey: XOnlyPubKey = XOnlyPubKey.fromhex(
    ...
    logger.info(
        f"Broadcast script transaction %s, attempting to spend it next", script_txid
    )

    timeout_blocks = test_case.tx_template.outputs[test_case.output_index][
        "spendingConditions"
    ][test_case.spending_condition_index].get("timeoutBlocks")
    if timeout_blocks:
        ...
        vout=[
            CTxOut(
                ...
                scriptPubKey=CScript(
                    [
                        OP_RETURN,
                        b"There must be some filler here or the TX will get rejected",
                    ]
                ),
            ),
        ],
    ...
    if print_witness:
        logger.info(
            "Witness elems:\n%s:",
            "\n".join(
                f"{i:03d}: {elem.hex()}" for i, elem in enumerate(full_witness_elems)
            ),
        )
