    ...
    *,
    ...
    for tx_input in tx_template.inputs[1:]:
        ...
        tx.vin.append(
            CTxIn(
                prevout=COutPoint(
                    bytes.fromhex(tx_input["txid"])[::-1], tx_input["vout"]
                ),
                nSequence=0,
            )
        )
        tx.wit.vtxinwit.append(
            CTxInWitness.deserialize(parse_hex_bytes(tx_input["witness"]))
        )
    ...
    if txid != tx_template.txid:
        raise ValueError(
            f"Constructed transaction has different txid {txid} than template {tx_template.txid}"
        )
...
def create_funded_transaction(
    *,
    ...
    fee_rate_sat_per_vb: int | Decimal,
    change_address: (
        str | CCoinAddress | None
    ) = None,  # Default to getting an address from the wallet
    lock_unspent: bool = True,
) -> CTransaction:
    """
    ...
    while True:
        ...
        try:
            ...
            raise OutOfFunds(
                f"Ran out of available UTXOs when trying to fund {tx_template.name}"
            )

        logger.debug(
            "Adding UTXO %s:%d to fund %s", utxo["txid"], utxo["vout"], tx_template.name
        )
    ...
    if not process_psbt_response["complete"]:
        raise ValueError(
            f"PSBT from walletprocesspsbt not complete: {process_psbt_response}"
        )
